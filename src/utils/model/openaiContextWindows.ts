...
function lookupExternalLimitMatches(
  ...
  const entries = readExternalLimits(envVarName, processEnv)
...
function lookupBuiltInLimit(
  ...
  const providerModel = processEnv.OPENAI_MODEL?.trim()
  if (providerModel && providerModel !== modelName) {
    const qualified = providerModel + ':' + modelName
    const qualifiedResult = lookupExactByKey(entries, qualified) ?? lookupPrefixByKey(entries, qualified)
    if (qualifiedResult !== undefined) {
      return qualifiedResult
    }
  }

  return lookupExactByKey(entries, modelName) ?? lookupPrefixByKey(entries, modelName)
}
...
function lookupLimit(
  ...
  return matches.exact ?? matches.prefix ?? lookupBuiltInLimit(entries, model, processEnv)
}
